...
async function main() {
  console.log("🚀 STARTING GROK AI TRADING AGENT REEL HYPERFRAMES RENDER");

  const compDir = path.join(process.cwd(), "public", "compositions", "grok-agent-editorial");
  const rawMp4 = path.join(compDir, "raw_render.mp4");
  const rendersDir = path.join(process.cwd(), "public", "renders");
  fs.mkdirSync(rendersDir, { recursive: true });
  const finalMp4 = path.join(rendersDir, "grok-agent-editorial-final.mp4");
  ...
  console.log("🎧 STEP 2: MUXING CLEAN DIALOGUE AUDIO (NO SOUND EFFECTS)");
