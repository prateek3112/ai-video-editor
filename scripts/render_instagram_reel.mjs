...
async function main() {
  console.log("🎬 Starting Render for Instagram Reel (Remotion)...");
  const entryPoint = path.join(process.cwd(), "components", "remotion", "index.ts");
  const rendersDir = path.join(process.cwd(), "public", "renders");
  await fs.mkdir(rendersDir, { recursive: true });

  const projectOutputPath = path.join(rendersDir, "toedit_instagram_final.mp4");
  const downloadsOutputPath = "/Users/prateekguglani/Downloads/toedit_instagram_final.mp4";
  ...
  console.log("🔍 Selecting composition 'InstagramReel'...");
  ...
  const previewFrames = [30, 120, 240, 330, 480, 720, 960];
  for (const f of previewFrames) {
    const stillPath = path.join(rendersDir, `preview_frame_${f}.png`);
    await renderStill({
      composition,
      serveUrl: bundled,
      output: stillPath,
